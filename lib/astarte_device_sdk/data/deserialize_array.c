...
#define DEFINE_PRIMITIVE_DESERIALIZER(FUNC_NAME, TYPE, MAP_TYPE, DATA_FIELD, EXTRACTOR)            \
    ...
    {                                                                                              \
        ...
        scope_defer(astarte_free)(array);                                                          \
        if (!array) {                                                                              \
...
static void cleanup_string_array(string_array_cleanup_ctx_t *ctx)
{
    if (ctx && ctx->array) {
        ...
        astarte_free(ctx->array);
    }
}

SCOPE_DEFER_DEFINE(cleanup_binblob_array, binblob_array_cleanup_ctx_t *);
SCOPE_DEFER_DEFINE(cleanup_string_array, string_array_cleanup_ctx_t *);
